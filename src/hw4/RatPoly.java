...
public final class RatPoly {
  ...
  public int degree() {
    // TODO: Fill in this method, then remove the RuntimeException
	  return this.terms.isEmpty()? 0 : terms.get(0).getExpt();
  }
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
    // TODO: Fill in this method, then remove the RuntimeException
	  ...
	  for (int i = 0; i < lst.size(); i++) {
		  RatTerm iTerm = lst.get(i);
		  if (newTerm.getExpt() == iTerm.getExpt()) {
			  lst.set(i, iTerm.add(newTerm));
			  // coeffs add up to 0 ==> remove
			  if (lst.get(i).isZero()) {
				  lst.remove(i);
			  }
			  return;
		  } else if (newTerm.getExpt() > iTerm.getExpt()) {
			  // before insert
			  // index  0  1  2   3   4  5  6
			  //      | .| .| .|  i | .| .| .|
			  // after insert
  ...
  public RatPoly mul(RatPoly p) {
    // TODO: Fill in this method, then remove the RuntimeException
	  ...
	  for (RatTerm t : this.terms) {
		  List<RatTerm> temp = new ArrayList<RatTerm>(p.terms);
		  scaleCoeff(temp, t.getCoeff());
		  incremExpt(temp, t.getExpt());
  ...
  public RatPoly div(RatPoly p) {
    // TODO: Fill in this method, then remove the RuntimeException
	  ...
	  RatPoly q = new RatPoly();  // for quotient polynomial
	  RatPoly r = new RatPoly(this.terms);  // remainder polynomial
	  while(!r.terms.isEmpty() && r.degree() >= p.degree()) {
		  // tempPoly = r_1 / p_1
		  RatPoly tempPoly = new RatPoly(r.terms.get(0).div(p.terms.get(0)));
		  // q = q + tempPoly
		  q = q.add(tempPoly);
		  // r = r - p * tempPoly
		  r = r.sub(p.mul(tempPoly));
	  }
	  return q;
  }
  ...
  public RatPoly differentiate() {
    // TODO: Fill in this method, then remove the RuntimeException
	  ...
	  for (RatTerm t : this.terms) {
		  if (!t.differentiate().isZero()) {
			  q.terms.add(t.differentiate());
		  }
	  }
	  return q;
  }
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
    // TODO: Fill in this method, then remove the RuntimeException
	  ...
	  RatPoly q = new RatPoly();
	  for (RatTerm t : this.terms) {
		  q.terms.add(t.antiDifferentiate());
	  }
  	  if (!integrationConstant.equals(RatNum.ZERO)) {
  		  sortedInsert(q.terms, new RatTerm(integrationConstant, 0));
  	  }
	  return q;
  }
  ...
  public double integrate(double lowerBound, double upperBound) {
    // TODO: Fill in this method, then remove the RuntimeException
	  ...
	  RatPoly r = antiDifferentiate(RatNum.ZERO);
