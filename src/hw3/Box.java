...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        // Returns false, if ball is already in the box or if the box
    	// is too full to contain the new ball.
        if (getVolume() + b.getVolume() > this.maxVolume || contains(b)) {
        	return false;
        }
	...
	public Iterator<Ball> getBallsFromSmallest() {
		// put balls in list
    	Iterator<Ball> bItr = iterator();
    	List<Ball> sortedballs = new ArrayList<Ball>();
    	while (bItr.hasNext()) {
    		sortedballs.add(bItr.next());
    	}
